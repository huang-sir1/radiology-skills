...
| Statistical test(s) used, and whether one- or two-sided | `radiology-stats` — name the test explicitly, as already required by `stat-reporting.md` |
...
- **Statistics fields** must name the exact test and sidedness — reuse the same test name
  already required in `radiology-stats`/`radiology-polishing/stat-reporting.md`; do not
  paraphrase differently in the two places.
