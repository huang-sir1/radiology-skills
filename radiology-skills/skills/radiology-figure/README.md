...
├── color-systems.md                选定唯一 palette（Okabe-Ito / NPG / Morandi），统一 color→meaning 映射
├── survival-figures.md             KM 完整性（曲线↔numbers-at-risk↔删失）、time-dependent ROC/calibration/DCA
├── figure-set-consistency.md       全套图 palette/字体/坐标统一，导出前与稿件表格和数据交叉核对
├── nature-figure-spec.md           Nature 系规格：89/183 mm 栏宽、小写 panel letters、RGB、图例字数上限、Extended Data/Source Data
├── figure-intent-and-render-qa.md  整图规划、标签拥挤/碰撞、最终尺寸渲染审查、source-data crosswalk
├── journal-family-visual-style.md  期刊家族视觉风格（Nature/npj、European Radiology 等）
└── backend-and-geometry-qa.md      Python vs R、白底/留白/边缘裁切/导出几何审计
```
