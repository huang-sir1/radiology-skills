...
- [ ] Foundation/SSL pretraining overlap is recorded as `VERIFIED_NO_OVERLAP`,
      `PARTLY_ASSESSABLE`, `CONTAMINATION_NOT_ASSESSABLE`, or `KNOWN_OVERLAP`, with the evidence and
      unit checked (patient/exam/series/derived image); unknown provenance is not recorded as absent.
- [ ] External task-cohort wording distinguishes geographic/institutional transport from unknown
      pretraining contamination; `KNOWN_OVERLAP` is not used for an untouched-test claim.
- [ ] Provider retention, prompt/RAG/output logging and model-access terms are checked before any PHI
      or controlled data are sent to a hosted foundation model.
...
Open `foundation-model-data-genealogy-and-privacy.md` for the complete genealogy, contamination and
threat-model gate.
