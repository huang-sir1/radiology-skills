...
## Access depth and claim support

Record `FULL_TEXT`, `ABSTRACT_ONLY`, `METADATA_ONLY` or `SOURCE_EXCERPT` for each inspected source,
with the exact section/page/table/block locator and version/correction status. Identifier resolution
establishes bibliographic identity, not substantive support. Metadata-only records cannot receive
`VERIFIED` or `SUPPORTED` for a scientific claim. An abstract or excerpt can support only what it
actually reports; missing Methods, subgroup, adjustment, limitation or supplementary details remain
`CANNOT_ASSESS`. Do not infer that an unreported method was absent from the full paper.

Before giving a material claim final verified support, check the source's correction, expression-of-
concern, retraction and preprint/version-of-record status. Record unavailable checks explicitly.
...
| Claim ID | Claim text | Claim type | Evidence/source + locator | Access depth / version status | DOI/PMID/accession | Status | Required action |
|---|---|---|---|---|---|---|---|
| BKG-01 | ... | background | ... | FULL_TEXT / checked | ... | VERIFIED | cite |
| NOV-01 | ... | novelty | ... | ABSTRACT_ONLY / checked | ... | PARTIAL | soften novelty |
| FIG-01 | ... | visual claim | source data / figure | SOURCE_EXCERPT / supplied version | n/a | UNSUPPORTED | correct figure or text |
