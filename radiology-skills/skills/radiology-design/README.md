...
| `references/modeling-route-decision.md` | 按 n / 事件数 / 标注成本 / 算力 / 时间选择建模路线（手工组学 / 深度特征 / 端到端 DL / foundation 微调） |
| `references/weak-result-pivot.md` | 弱结果（如 AUC ~0.7）的五条合规出路与英文声明模板 |
| `references/ai-radiogenomics-12-24-roadmap.md` | 12-24 个月影像 AI / 深度影像组学 / radiogenomics 分阶段发表与转化路线图 |

## 下游衔接

`radiology-frontier`（是否新颖/可发表）· `radiology-stats`（样本量、EPV、power）· `radiology-radiomics` / `radiology-deep-learning` / `radiology-radiogenomics`（具体方法设计）· `radiology-annotation`（mask SOP）· `radiology-reporting`（报告规范）· `radiology-ethics`（伦理/共享可行性）· `radiology-translation`（reader study / prospective plan）· `radiology-grant`（设计转为基金申请）· `radiology-pipeline`（项目状态、决策锁定与下游 staleness）。
