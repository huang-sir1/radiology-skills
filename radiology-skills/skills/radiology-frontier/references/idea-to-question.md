...
Minimum evidence for the intended claim: [validation type; calibration/reader/DCA only if applicable]
...
State the uncertainty resolved, comparison or failure boundary in one sentence. A useful question
can test independent transport, replication, robustness, lack of incremental value, lower resource
burden, or a justified equivalence/non-inferiority hypothesis. It need not beat a baseline. Prespecify
the estimand and any clinically justified margin before results; a nonsignificant superiority test
does not establish equivalence or non-inferiority. Retain null and adverse findings in the question's
value proposition and reporting plan.
