...
## Candidate trajectories to verify

The following are useful search trajectories, not a verified current ranking or a requirement that
every project replace a simpler model. Verify the trajectory against current primary literature,
the user's resources and the uncertainty being resolved:
...
A corresponding radiogenomics trajectory to investigate is:
...
|---|---|---|
| Generalisability | Validation with explicit patient/site/time/protocol independence for the transport claim | Random mixed splits or federated participation presented as external validation; a participating training site is not unseen |
...
| Multimodal fusion | Test incremental value beyond clinical/molecular/pathology alone, with missing-modality and matched-sample plans | Claiming gain without a fair comparator or hiding null/negative incremental value |
...
| Clinical-value evidence | Choose reader, silent, active-impact or decision analysis for the intended claim; DCA is conditional on a defensible threshold-to-action question | Treating DCA, silent deployment and observed patient benefit as interchangeable |

## Candidate directions, without a fixed priority

| Candidate | Direction | Best-fit data |
|---|---|---|
...
Concrete papers, current frontier ranking, dataset status and regulatory facts must be verified before
a current recommendation. `radiology-search` may supply the evidence; an unresolved handoff leaves
the shortlist provisional. Prioritize scientific value, feasibility and falsifiability for this user.
