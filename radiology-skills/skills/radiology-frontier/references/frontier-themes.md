...
| **Multimodal fusion** | Combine imaging + clinical + pathology/molecular + text | Matched modalities on the same patients for patient-level fusion claims | Unfair comparators or hidden null/negative incremental value; absent missing-modality handling |
...
| **Uncertainty / OOD / safety** | Calibrated confidence, out-of-distribution detection | A defined deployment distribution to test against | Uncertainty reported but never acted on (→ `radiology-deep-learning/interpretability-uncertainty.md` for execution) |
| **Interpretability / mechanism** | Saliency, concept attribution, radiogenomics | For mechanism: **matched** imaging+omics (→ radiology-radiogenomics) | Over-interpreting saliency maps as biology (→ `radiology-deep-learning/interpretability-uncertainty.md` for execution) |
...
| **Liquid biopsy / ctDNA ↔ imaging** | Correlate circulating tumour DNA (burden, variant fraction, methylation signal) or other liquid-biopsy analytes with imaging phenotype/burden/habitat | Matched imaging + plasma-sampling timepoint per patient (same timing-mismatch discipline as tissue radiogenomics) | Treating a single plasma draw as representative of whole-body burden; ignoring assay LOD/sensitivity at low tumour fraction — bound claims the way `radiology-radiogenomics/sample-to-image-mapping.md` bounds tissue-based claims |
| **Pathology-foundation-model × imaging fusion** | Joint embeddings from a pretrained histopathology foundation model and a radiology-imaging encoder/radiomics | Matched whole-slide imaging + radiology imaging per patient, both usable-quality | Small matched-n overfitting (same risk profile as radiogenomics); asserting a joint embedding is "explainable" without checking what it actually weights (→ `radiology-deep-learning/interpretability-uncertainty.md`) |
...
- Adding fusion complexity without testing whether it improves a prespecified decision or resolves
  a meaningful uncertainty; a well-designed finding of no incremental value is still informative.
