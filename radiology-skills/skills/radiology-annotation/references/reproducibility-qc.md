...
- Define the **subset** (all cases, or a random sample with a stated size and rationale) —
  size it so the ICC 95% CI is acceptably narrow (precision-based calculation →
  `radiology-stats`), not an arbitrary 30%.
...
*"Of [N] extracted features, [K] with inter-observer ICC > 0.80 (two-way random-effects,
