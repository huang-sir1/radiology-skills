...
- **Outcome information and model complexity** — do not use a universal events-per-variable
  cutoff. Plan from the estimand, censoring/event process, candidate-parameter complexity,
  anticipated shrinkage/optimism and desired precision; imaging/omics signatures must avoid
  fitting a high-dimensional raw feature set without appropriate reduction and validation.
...
calibration. The pre-specified Schoenfeld-residual assessment provided no evidence of a
proportional-hazards violation (global P = .34); this non-significant test does not prove the
assumption, and graphical diagnostics were also examined."*
