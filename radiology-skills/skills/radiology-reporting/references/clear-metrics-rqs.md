...
- **RQS / RQS 2.0** — *quality score*. Original Radiomics Quality Score (Lambin et al. 2017,
  **16 components, max 36 points**). **RQS 2.0** (2025) expands to **42 criteria** scored per
  track (hand-crafted max **56** / deep-learning max **52**) and maps them onto **Radiomics
  Readiness Levels (RRL 1–9)** for clinical translation (calculator: radiomics.world/rqs2).
  Widely cited; some items now superseded by METRICS, so
...
## Output
`Tool | Item/Category | Status or score | Location | Fix | Owner | Closure evidence`. Report CLEAR as
PRESENT/PARTIAL/MISSING per item; METRICS as category scores + total %; RQS 1.0 as
points/36 (RQS 2.0 as points/56 hand-crafted or /52 deep-learning + RRL mapping) if requested.
