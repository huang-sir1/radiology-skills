...
- **Scanner/protocol harmonisation** — name the method, implementation, fitting data, covariates,
  and whether validation batch levels were represented during fitting. “Training only” prevents
  leakage but does not let ordinary/transductive ComBat transform a wholly unseen site. Report
  raw-scale external results, any pre-specified unseen-batch method, and external-data adaptation as
  distinct analyses; do not claim biological signal was preserved without evidence.
...
compliance? · segmentation ICC + filtering threshold? · harmonisation fit data + represented/unseen
batch support + raw/adapted external analysis?
