# Nature-family manuscript shape (Nature Medicine / Nature Biomedical Engineering / Nature Communications / npj Digital Medicine, etc.)

Use this file instead of `structured-abstract.md`/`article-architecture.md` when the target is a
**Nature-portfolio-family** venue rather than _Radiology_. Cell Reports Medicine and Cancer Cell are
Cell Press journals and must route through `cell-jama-wiley-shape.md`, not this profile. The argument-first discipline, the
...
| Methods placement | Inline, ordered section (see methods.md) | Placement differs by tier: **Nature flagship journals** (Nature, Nature Medicine, Nature Biomedical Engineering) place Methods **after References**, online-only, not counted in the main word limit; **Nature Communications and the npj series** keep Methods in the main article flow, after the Discussion and **before References**. Either way it still needs the same content (design, cohort, technique, model/statistics) and the same reporting-guideline coverage; check the specific venue's current instructions for placement |
| Display items in main text | Figures/tables within journal limits | A **small number of main display items** (figures/tables); everything else goes to **Extended Data** (peer-reviewed, published) or **Supplementary Information** (not typeset) — verify the current cap live; hand off figure-count planning to `radiology-figure`/`nature-figure-spec.md` and `radiology-data` |
| Reference style | Numbered (AMA style) per _Radiology_ house style — there is no author–year option | Numbered, in order of first citation (Vancouver-style superscripts) — hand off to `radiology-citation`/`export-formats.md`, section "Venue reference styles (Vancouver/ICMJE)" |
| Statistics disclosure | Reported in Methods/Results prose | Prose **plus** a separate **Reporting Summary** for life-sciences submissions (→ `radiology-reporting/nature-reporting-summary.md`) |
...
- **Discussion** keeps the same discipline as `discussion.md` (key finding first, no new results,
  ...
  scrutinised harder (→ `radiology-deep-learning/interpretability-uncertainty.md` for AI-specific
...
4. **`Display-item plan`** — which results go to main figures/tables vs Extended Data/Supplementary
   Information (→ [Nature figure specification](../../radiology-figure/references/nature-figure-spec.md),
   `radiology-data`).
5. **`Reporting check`** — checklist items **plus** whether a Reporting Summary is required for this
   venue (→ [Nature reporting summary](../../radiology-reporting/references/nature-reporting-summary.md)).

## Handoffs
- Figure sizing, panel-letter case, Extended Data figure limits →
  [radiology-figure Nature specification](../../radiology-figure/references/nature-figure-spec.md).
...
- Reporting Summary / Editorial Policy Checklist →
  [radiology-reporting Nature summary](../../radiology-reporting/references/nature-reporting-summary.md).
