...
- [ ] **Harmonisation (ComBat or another method)** avoids outcome/test leakage **and** states batch
  support: frozen training parameters are applied only to represented batch levels with a compatible
  implementation. A wholly unseen external site is not silently transformed with ordinary/
  transductive ComBat; raw-scale evaluation is retained or adaptation is labelled separately.
