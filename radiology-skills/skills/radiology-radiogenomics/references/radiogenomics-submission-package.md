# Imaging-mechanism submission and rebuttal package

Use this only when the declared claim connects imaging with a molecular, cellular, spatial or
pathology mechanism layer. The goal is to make the paper easy for an editor, methods reviewer,
mechanism reviewer and imaging reviewer to verify. For imaging-only or mechanism-only work, use the
scope-specific package instead; do not request a nonexistent modality.
...
- Omics: GEO/SRA/BioStudies/GDC/cBioPortal/dbGaP/EGA as applicable.
