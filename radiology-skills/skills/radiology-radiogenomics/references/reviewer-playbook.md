# Imaging-mechanism reviewer and revision playbook

Use this only when the declared claim connects imaging with a molecular, cellular, spatial or
pathology mechanism layer. For imaging-only or mechanism-only work, use the scope-specific reviewer
playbook; do not ask the study to supply a modality outside its declared scope.
...
| Batch drives the association | Scanner/sequencing confounding | Report batch variables and sensitivities; apply frozen harmonisation only to represented batch levels. Ordinary ComBat has no parameters for a wholly unseen center; estimating them there is adaptation/transductive, not untouched external validation |
...
Use the evidence and repair fields defined in `constructive-manuscript-review-chain.md`. At minimum
return comments in this structure:

`Finding ID | Class | Review dimension/lens | Severity | Obligation | Typed evidence anchor |
Observed problem | Governing criterion | Why it matters | Claim consequence | Minimum feasible remedy |
Optional stronger route | Cost/trade-off | Closure evidence | Confidence/scope limit | Routed skill`

Reviewer personas are lenses applied by one audit, not claims of independent reviewers:
